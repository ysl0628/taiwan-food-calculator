...
const CalculatorView: React.FC<CalculatorViewProps> = ({
  ...
  const recordTotals = Object.values(dailyRecord)
    .flat()
    .reduce(
      (acc, item) => ({
  ...
  const totalLoggedItems = Object.values(dailyRecord).reduce(
  ...
  const fetchData = async (pageNum: number, reset: boolean) => {
    ...
    try {
      ...
      if (reset && (searchTerm || selectedCategory !== "All")) {
        capturePH("food_search", {
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] lg:h-screen bg-slate-50">
              ...
              {totalLoggedItems === 0 ? (
                ...
                MEAL_TIMES.map((meal) => {
                  ...
                  const mealCalories = items.reduce(
                    (sum, item) => sum + item.cal * item.quantity,
                    0
                  );
      ...
      <div className="hidden lg:block flex-shrink-0 w-full bg-slate-50/95 backdrop-blur-md border-b border-slate-200 shadow-sm z-30">
        <div className="max-w-[1600px] mx-auto px-4 md:px-6 py-4">
          <ProgressDashboard
            current={combinedTotals}
            target={dietPlan}
            onAnalysis={onGoToAnalysis}
          />
        </div>
      </div>
            ...
            {hasMore && (
              <div className="mt-4 mb-8 text-center pb-20 lg:pb-0">
                <button
                  onClick={loadMore}
                  disabled={isLoading}
                  className="px-6 py-3 bg-white border border-slate-200 text-slate-600 font-medium rounded-xl hover:bg-slate-50 flex items-center gap-2 mx-auto"
                >
                  {isLoading ? (
